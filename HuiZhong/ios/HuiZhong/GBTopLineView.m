...
- (void)setVerticalShowDataArr:(NSMutableArray *)dataArr
{
    ...
    NSDictionary  *model = _dataArr[count];
  
//    model.type = @"1";
  [self.currentBtn setTitle:[model objectForKey:@"type"] forState:UIControlStateNormal];
    self.currentLabel.text = [model objectForKey:@"title"] ;
}
...
//        GBTopLineViewModel *hienModel = _dataArr[count];
      ...
      //    model.type = @"1";
      [self.hidenBtn setTitle:[model objectForKey:@"type"] forState:UIControlStateNormal];
      self.hidenLabel.text = [model objectForKey:@"title"] ;
...
- (void)createCurrentView
{
    ...
    self.currentBtn.frame = CGRectMake(12,10, 30, self.currentView.frame.size.height-20);
    ...
    //内容标题
    self.currentLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.currentBtn.frame.origin.x+self.currentBtn.frame.size.width+10+3,3, self.currentView.frame.size.width-(self.currentBtn.frame.origin.x+self.currentBtn.frame.size.width+10+10), self.currentView.frame.size.height)];
    self.currentLabel.text = [model objectForKey:@"title"];
    ...
    self.currentLabel.numberOfLines = 2;
...
- (void)createHidenView
{
    ...
    self.hidenBtn.frame = CGRectMake(12, 10, 30, self.hidenView.frame.size.height-20);
    ...
    //内容标题
    self.hidenLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.hidenBtn.frame.origin.x+self.hidenBtn.frame.size.width+10+3,3, self.hidenView.frame.size.width-(self.hidenBtn.frame.origin.x+self.hidenBtn.frame.size.width+10+10), self.hidenView.frame.size.height)];
    ...
    self.hidenLabel.numberOfLines = 2;
